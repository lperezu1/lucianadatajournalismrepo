...
```{r}
  ...
  labs(x="Year", y="Total pills", title="In Logan County opioids increase from 2006 through 2008, but remain in the 6,000,000 range through 2012", subtitle = "Total pills shipped to Logan County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
...
ggplot(wv_pills_per_year) +
  ...
  labs (x="Year", y="Total pills", title= "Opioids sent to six large West Virginia counties", subtitle = "Total pills per year shipped to Kanawha, Cabell, Mingo, Raleigh, Logan and Wood" , caption = "Source: DEA ARCOS database, via Washington Post",fill="County")+
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012)) +
  scale_y_continuous(labels = comma) +
...
#county_pills_per_person <- get_acs(geography = "county", variables = c("B19013_001"), survey="acs5", year = 2012)

```


```{r}
# Maryland counties pills per person rate (not total pills, but pills per person) for a county in 2012 and the county's median household income in 2012.


maryland_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "MD", year=="2012") %>%
  select(buyer_county, dosage_unit, countyfips) %>% 
  mutate(GEOID = countyfips)
...
# maryland population 2012 
maryland_population_2012 <- county_population(key = key) %>%
  clean_names() %>%
  filter(buyer_state == "MD", year=="2012") %>%
  select(buyer_county, countyfips, population)
...
#maryland 2012 is pills inner joined with population USING COUNTY FIPS 

maryland_2012 <- maryland_pills_2012 %>%
  inner_join(maryland_population_2012, by=("countyfips")) %>%
  select (countyfips,dosage_unit, population, buyer_county.x ) %>%  
  mutate(pills_per_person = dosage_unit/population) %>% 
  mutate ( buyer_county = buyer_county.x )
...
#final data pills/ median income/ 

#Md_pills_per_person_median_income <- maryland_2012 %>%
  #inner_join(md_county_median_household_income, by =("countyfips")) %>%  
  #mutate(buyer_county = buyer_county.x) %>%
 # select ( countyfips,buyer_county,  pills_per_person, estimate)
...
```{r}
#ggplot(Md_pills_per_person_median_income) +
 # geom_point(aes(pills_per_person,estimate)) +
  #labs(x="pills_per_person", y="median_household_income", title=" Counties number of opioids per person relative to median household income", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  #scale_y_continuous(labels = comma) +
  ...
  #geom_text_repel(aes(pills_per_person, estimate, label= buyer_county),
   #               subset(Md_pills_per_person_median_income, estimate > 80000))
...
#pills_all_years <- arcos_county_pills_per_year %>%
 # select(buyer_state, buyer_county, dosage_unit, countyfips)


# population 2012 
#county_population_2012 <- county_population(key = key) %>%
 # clean_names()  %>%
 # select(buyer_state, buyer_county, countyfips, population,year, population)


#pills_per_person_county_level <- pills_all_years %>%
#inner_join(county_population_2012, by=("countyfips")) %>% 
#select (countyfips , buyer_state.x , buyer_county.x, population, dosage_unit) %>%
#mutate (pills_per_person = dosage_unit/population) %>% rename (buyer_state =buyer_state.x, buyer_county = buyer_county.x )


#ggplot(pills_per_person_county_level) +
 # geom_point(aes(population,dosage_unit)) +
  #labs(x="population", y="dosage_unit", title=" Counties number of opioids per person ", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
 # scale_y_continuous(labels = comma) +
 # scale_x_continuous(labels = comma) + 
 # theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
 # geom_smooth(aes(population,dosage_unit), method = "lm", se = FALSE) +
  # geom_text_repel(aes(dosage_unit, population, label= buyer_county),
                #   subset(pills_per_person_county_level, population > 80))
