...
# annual population for states between 2006 and 2012 

state_population <- state_population( key = key)


#  total pills for each manufacturer in a state 

#total_manufacturers_state <- total_manufacturers_state (key = key)


#  total pills for each pharmacy in a state

 #total_pharmacies_state <- total_pharmacies_state(key = key)
 
 
 #Get total pills for each distributor in a state 
 
 
 #total_distributors_state <-total_distributors_state (key = key)
...
```{r}
# ideas -Medicare state data of users buying opioids compared with data avaialble through arcos ( senior opioid use.  Would need https://projects.propublica.org/checkup/ and pills per person in a state.  https://www.propublica.org/article/risky-overused-medications-prescribed-far-less-often-in-the-aloha-state
 
 
 
 # use of opioids by generations -ages of people who used opioids most- most affected? By zip-code for example ? 
  
 
 
 #medicaid - state with information available regarding medicaid, and prescriptions for opioids. Compare rate of opioid use per person compared to opioid use per person through the arcos database. Compared to national rate? maybe more than 1 state?  Would need population for the state/ opioid use through medicaid/ opioiod use per person through arcos. 


# IDEALLY ENROLLMENT IN MEDICAID PER STATE- AND OPIOID UTILIZED IN THAT STATE- OR PRESCRIBED BY PHARMACY/ DISPENSARY/ DOCTOR/ PRACTITIONER ETC. 

#OR SPENDING ON MEDICAID OPIOID OVERDOSE DATASET - IN WV FOR EXAMPLE (THIS SEEMS TO EXIST) , INCREASE IN COSTS OVER YEAR///  compare with opioid pills sols in different locations (ARCOS)
