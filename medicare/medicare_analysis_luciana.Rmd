...
opioid claims per 65+ by zipcode vs pills per person rate by zipcode (arcos)
Join by zipcode
test to see if there is any pattern. 

```{r}

medicare_opioids_zip_count <- medicare_edited %>% group_by(nppes_provider_zip_code) 
...
estimate_total_medicare_male <- get_acs(geography = "county",
              variables = "C27006_010") 


estimate_total_medicare_female <- get_acs(geography = "county",
              variables = "C27006_010")
...
medicare_population_opioids_zip <-medicare_opioids_zip_count %>%
  ...
  filter (total_medicare ! =0, total_opioids_)
...
medicare_edited %>%
  ...
  summarise(total_claims = sum(total_claim_count),
            total_opioid_claims = sum(opioid_claim_count),
            long_acting_opioid_claim_count = sum(long_acting_opioid_claim_count),
            short_acting_opioid_claim_count) %>%
  mutate(pct_opioid_claims = total_opioid_claims/total_claims)
