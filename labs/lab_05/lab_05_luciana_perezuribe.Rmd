...
mingo %>%
  group_by(transaction_year)%>%
  summarise(total_shipments = n(),total_pills_per_year = sum(dosage_unit))%>%
  mutate(total_pills_shipped=sum(total_pills_per_year))

#pills shipeed to Mingo County per year incremented from 2006-2008 but decreased afterwards. 
