...
```{r}
# Comment 578 of the records in the pills table without a match in population table are in PR. This can be problemtic in the analysis as it will exclude pr and not allow a full picture. PR could have been sent a considerable amount of opioids per person for example .
```
...
```{r}
#str_sub grabs first to characters in countyfips and creates statefips 
...
#Show pills per person instead of population 
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per person',title="West Virginia, South Carolina have most pills per person", subtitle = "2017 pills per person, U.S. Census", caption = "Source: U.S. Census ACS") +
...
# facet_wrap(~year, nrow=2) + says create one chart for each value and organize into two rows 
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="", subtitle = "2006-2012 Pills per person in U.S. Statest, U.S. Census", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increase continously from 2006-2012, states in the East see substantial increases", subtitle = "2006-2012 Pills per person in U.S. Statest, U.S. Census", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
```{r}
#single interactive map based on pills per person in each county in 2009 
...
#Error in leaflet::colorNumeric(palette = palette, domain = domain, na.color = na.color, : Wasn't able to determine range of domain 
``` 
...
pills_population_maryland_counties <- pills_population %>%
...
geo_pills_population_maryland_counties<- county_geodata_shifted %>%
 inner_join(pills_population_maryland_counties, by=c("GEOID" = "countyfips"))
...
```{r}
geo_pills_population_maryland_counties %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increase continously from 2006-2012 in Maryland Counties,  decrease slightly in 2012", subtitle = "2006-2012 Pills per person in U.S. Statest, MD counties", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
